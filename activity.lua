...
function main()
    ...
    if DEBUG_MODE == true then atext(("Âêëş÷åí ğåæèì îòëàäêè. (Âåğñèÿ ñêğèïòà: %s. Íîìåğ ñáîğêè: %s)"):format(thisScript().version, thisScript().version_num)) end
    ...
    if pInfo.info.day ~= day and tonumber(os.date("%H")) > 4 then
      ...
      if tonumber(pInfo.info.thisWeek) ~= tonumber(os.date("%W")) then
        ...
        debug_log("Íîâàÿ íåäåëÿ. Îáíóëÿåì âñå ïåğåìåííûå")
        pInfo.info.thisWeek = os.date("%W")
      end
      debug_log("Íîâûé äåíü. Îáíóëÿåì äåíü. weekOnline = "..pInfo.info.weekOnline)
      pInfo.info.day = day
      pInfo.info.dayPM = 0
...
function sampevents.onServerMessage(color, text)
  ...
  if text:match("Ââåäèòå: %(/a%)dmin") and not sInfo.isAlogin then -- Ïğîâåğêà íà àäìèíêó ïğè ïåğåçàãğóçêå ñêğèïèòà â èãğå
    sInfo.isALogin = true
    sInfo.sessionStart = os.time()
    return false
  end
